...
from crewai.crew import Agent, LLM, Crew, Task
...
class BlogContentMakerFlow(Flow[BlogContentMakerState]):
    ...
    def manage_seo(self):
        ...
        self.state.score_manager = result.pydantic

    @router(manage_seo)
    def manage_score_router(self):
        if self.state.score_manager is None:
            raise ValueError("score_manager is required")
    ...
    def _save_to_markdown(self):
        ...
        **SEO 점수**: {self.state.score_manager.score}
        ...
        ## SEO 분석
        **점수**: {self.state.score_manager.score}
...
flow.kickoff({"topic": "IREN std. 기업 평가"})
# flow.plot()
